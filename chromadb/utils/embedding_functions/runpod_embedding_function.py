...
class RunPodEmbeddingFunction(EmbeddingFunction[Documents]):
    ...
    def __call__(self, input: Documents) -> Embeddings:
        ...
        # Process documents (could be batched for efficiency)
        for document in input:
            try:
                ...
                run_request = self.endpoint.run(input_payload)
                ...
                if output and "data" in output:
                    ...
                    raise ValueError(f"Unexpected output format. Expected 'output.data[0].embedding' or 'data[0].embedding', got: {output}")
                ...
                # You might want to handle this differently - skip, retry, or fail
                raise RuntimeError(f"Failed to generate embedding for document: {e}")
    ...
    def list_available_models(self) -> List[str]:
        ...
        models = []
        try:
            run_request = self.endpoint.run({ "input": { "openai_route": "/v1/models" } })
            print(run_request)
            status = run_request.status()
